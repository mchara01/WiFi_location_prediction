...
def read_full_dt(file_path, test_proportion, validation_proportion):
    """This function reads the dataset from a given file path, splits it into training and test samples and returns the result along with their classes.
...
def main_method():
    ...
    if os.path.isfile(clean_dataset_file_path):
        x, y, classes = read_full_dt(clean_dataset_file_path, testing_proportion, validation_proportion)
...
if __name__ == "__main__":
    ...
    y_pred_clean = clean_decision_tree.predict(x_test_clean)
    y_pred_noise = clean_decision_tree.predict(x_test_noise)
